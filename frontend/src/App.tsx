...
import { 
  ...
  ComposedChart, Radar, LineChart, Line, PieChart, Pie, Cell
} from 'recharts';
import { 
  TrendingUp, TrendingDown, BarChart3, PieChart as PieChartIcon, 
  Target, Shield, AlertTriangle, Award, Calendar, Clock, 
  DollarSign, Activity, Users, LogOut, Plus, 
  Edit, Trash2, Filter, RefreshCw, Brain, Zap, 
  CheckCircle, XCircle, AlertCircle, ArrowUpRight, ArrowDownRight,
  LineChart as LineChartIcon, RadarIcon, Sparkles, Crown,
  Star, Lock, Unlock, Globe
} from 'lucide-react';
import { createClient, AuthChangeEvent, Session } from '@supabase/supabase-js';

// Supabase client setup
const supabaseUrl = 'https://fsicauceosmdrhxmvreu.supabase.co';
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZzaWNhdWNlb3NtZHJoeG12cmV1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTgxMDI5OTksImV4cCI6MjA3MzY3ODk5OX0.J_Dx9SLkzffTFcDhxMix56cmtpM4710nqafnyP5BLhk';

if (!supabaseKey) {
  console.error('Supabase key is missing. Please add REACT_APP_SUPABASE_ANON_KEY to your environment variables.');
}
...
function TradingJournalApp() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'trades' | 'analytics' | 'patterns' | 'insights'>('dashboard');
  ...
  const convertSupabaseUser = (supabaseUser: any): User => {
    return {
      ...
      full_name: supabaseUser.user_metadata?.full_name || supabaseUser.user_metadata?.name || supabaseUser.email || 'User',
  ...
  const apiCall = async <T,>(endpoint: string, options?: RequestInit): Promise<T> => {
    try {
      ...
      if (authState.session?.access_token) {
        (headers as Record<string, string>)['Authorization'] = `Bearer ${authState.session.access_token}`;
      }
  ...
  const fetchEntries = async () => {
    try {
      const response = await apiCall<{trades: JournalEntry[]}>(`/trades?limit=1000`);
  ...
  const fetchAdvancedStats = async () => {
    try {
      const response = await apiCall<{stats: AdvancedStats}>(`/analytics/advanced-stats`);
      if (response.stats) {
  ...
  const fetchTimeSeriesData = async () => {
    try {
      const response = await apiCall<{data: TimeSeriesData[]}>(`/analytics/time-series?period=${selectedPeriod}`);
      if (response.data) {
  ...
  const fetchPatternData = async () => {
    try {
      const response = await apiCall<{patterns: PatternData}>(`/analytics/patterns`);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('provider is not enabled')) {
        setError('Google OAuth is not configured. Please enable Google provider in Supabase Dashboard → Authentication → Providers');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!apiConnected || !authState.user) {
      alert('API connection not available or user not authenticated. Please refresh the page or sign in again.');
      return;
    }
    ...
    try {
      ...
      const strategy = formData.strategy || calculateStrategy(entryDateTime, exitDateTime);
  ...
  const stats = statistics || {
    ...
    avg_pnl: entries.length > 0 ? (entries.reduce((sum, entry) => sum + (entry.p_and_l || 0), 0) / entries.length) : 0,
  ...
  const prepareComprehensiveRadarData = () => {
    ...
    return [
      ...
      {
        metric: 'Calmar Ratio',
        value: Math.min(Math.max(advancedStats.risk.calmarRatio * 20 + 50, 0), 100),
        fullMark: 100
      },
      {
        metric: 'Kelly %',
        value: Math.min(Math.max(advancedStats.performance.kellyPercentage + 50, 0), 100),
        fullMark: 100
      }
    ];
  };
  ...
  const generateAdvancedAnalytics = async () => {
    ...
    try {
      const analyticsData = {
        ...
        timeRange: {
          firstTrade: entries.length > 0 ? entries[entries.length - 1].entry_dt : null,
        ...
        riskMetrics: {
          ...
          avgWin: entries.filter(e => (e.p_and_l || 0) > 0).length > 0 
            ? entries.filter(e => (e.p_and_l || 0) > 0).reduce((sum, e) => sum + (e.p_and_l || 0), 0) / entries.filter(e => (e.p_and_l || 0) > 0).length
            : 0,
          avgLoss: entries.filter(e => (e.p_and_l || 0) < 0).length > 0 
            ? entries.filter(e => (e.p_and_l || 0) < 0).reduce((sum, e) => sum + (e.p_and_l || 0), 0) / entries.filter(e => (e.p_and_l || 0) < 0).length
            : 0
        }
      };
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            },
            {
              ...
              RISK METRICS:
              - Maximum Single Loss: ₹${analyticsData.riskMetrics.maxLoss.toFixed(2)}
              - Maximum Single Win: ₹${analyticsData.riskMetrics.maxWin.toFixed(2)}
              - Average Winning Trade: ₹${analyticsData.riskMetrics.avgWin.toFixed(2)}
              - Average Losing Trade: ₹${analyticsData.riskMetrics.avgLoss.toFixed(2)}
              
              BEST PERFORMING STRATEGY: 
              ${analyticsData.bestPerformingStrategy?.strategy} - ₹${analyticsData.bestPerformingStrategy?.pnl.toFixed(2)} from ${analyticsData.bestPerformingStrategy?.trades} trades
              
              TIME RANGE: 
              ${analyticsData.timeRange.firstTrade} to ${analyticsData.timeRange.lastTrade}
      ...
      try {
        ...
        setAnalytics({
          executive_summary: {
            overall_performance: `Based on ${analyticsData.totalTrades} trades with a ${analyticsData.winRate.toFixed(1)}% win rate and total P&L of ₹${analyticsData.totalPnL.toFixed(2)}`,
            key_strengths: [
              analyticsData.winRate > 50 ? "Positive win rate indicates good trade selection" : "Room for improvement in trade selection",
              analyticsData.totalPnL > 0 ? "Profitable trading overall" : "Focus on improving profitability",
              `Active in ${analyticsData.strategies.length} different strategies showing diversification`
            ],
            critical_weaknesses: [
              analyticsData.winRate < 50 ? "Win rate below 50% needs improvement" : "Consider optimizing winning trade size",
              analyticsData.totalTrades < 20 ? "Limited sample size for statistical significance" : "Ensure consistent execution",
              "Consider implementing stricter risk management rules"
            ],
            trader_profile: analyticsData.strategies.includes('scalping') ? "Active short-term trader" : "Swing/position trader"
          },
          performance_metrics: {
            profit_factor: analyticsData.riskMetrics.avgWin > 0 && analyticsData.riskMetrics.avgLoss < 0 
              ? (analyticsData.riskMetrics.avgWin / Math.abs(analyticsData.riskMetrics.avgLoss)).toFixed(2) 
              : "Insufficient data",
            return_on_investment: `${((analyticsData.totalPnL / Math.max(analyticsData.totalTrades * 1000, 1)) * 100).toFixed(2)}%`
          },
          actionable_recommendations: {
            immediate_actions: [
              `Your current win rate is ${analyticsData.winRate.toFixed(1)}%. ${analyticsData.winRate > 50 ? 'Focus on increasing position sizes for winning trades.' : 'Review and improve entry criteria.'}`,
  ...
  const prepareChartData = () => {
    ...
    const pnlDistribution = [
      { name: 'Profitable Trades', value: entries.filter(e => (e.p_and_l || 0) > 0).length, fill: '#10b981' },
      { name: 'Losing Trades', value: entries.filter(e => (e.p_and_l || 0) < 0).length, fill: '#ef4444' },
      { name: 'Break-even Trades', value: entries.filter(e => (e.p_and_l || 0) === 0).length, fill: '#8b5cf6' }
    ];
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center font-['Inter'] antialiased">
        <Card className="w-full max-w-md mx-4 border-0 shadow-2xl bg-white/80 backdrop-blur-sm">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-slate-600 font-medium">Initializing TradingJournal Pro...</p>
            <p className="text-sm text-slate-500 mt-2">Connecting to Supabase and loading analytics</p>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center p-4 font-['Inter'] antialiased">
        <div className="w-full max-w-lg">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-lg text-slate-600 font-medium">Elite Trading Analytics Platform</p>
            <p className="text-slate-500">Powered by AI insights and advanced analytics</p>
          </div>
          ...
          <Card className="border-0 shadow-2xl bg-white/90 backdrop-blur-xl">
            <CardHeader className="text-center pb-8 pt-8">
              ...
              <CardDescription className="text-slate-600 text-base leading-relaxed">
                Access your professional trading dashboard with AI-powered insights, advanced analytics, and comprehensive performance tracking
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-8 px-8 pb-8">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-sm">
                  <span className="px-4 bg-white text-slate-500 font-medium">Premium features included</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-6 text-center text-sm">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs text-slate-500">Advanced Charts</span>
                </div>
                <div className="flex flex-col items-center">
              ...
              <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-4 border border-indigo-100">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-semibold text-slate-700">All Premium Features Included</span>
                  <Star className="w-4 h-4 text-amber-500 fill-current" />
                </div>
                <p className="text-xs text-slate-600 text-center">No subscriptions • No hidden fees • Full access forever</p>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="text-center mt-8 space-y-2">
            <p className="text-slate-600 font-medium">Trusted by professional traders worldwide</p>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-3 bg-slate-50 rounded-xl px-4 py-2">
                ...
                <div className="text-sm">
                  <div className="font-semibold text-slate-800">{authState.user.full_name}</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
              <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold mb-1">{stats.total_trades}</div>
                      <div className="text-blue-100 font-medium">Total Trades</div>
                    </div>
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-amber-500 to-orange-600 text-white">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold mb-1">{stats.open_trades}</div>
                      <div className="text-amber-100 font-medium">Open Trades</div>
                    </div>
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-purple-500 to-violet-600 text-white">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold mb-1">{stats.closed_trades}</div>
                      <div className="text-purple-100 font-medium">Closed Trades</div>
                    </div>
              ...
              <Card className={`border-0 shadow-xl text-white ${
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold mb-1">₹{stats.total_pnl.toFixed(2)}</div>
                      <div className={`font-medium ${stats.total_pnl >= 0 ? 'text-green-100' : 'text-red-100'}`}>
                        Total P&L
                      </div>
                    </div>
                    {stats.total_pnl >= 0 ? (
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-cyan-500 to-blue-600 text-white">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold mb-1">{stats.win_rate.toFixed(1)}%</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <Card className="border-0 shadow-xl bg-white hover:shadow-2xl transition-shadow duration-300">
                  <CardContent className="p-8">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-2">Profit Factor</p>
                ...
                <Card className="border-0 shadow-xl bg-white hover:shadow-2xl transition-shadow duration-300">
                  <CardContent className="p-8">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-2">Sharpe Ratio</p>
                ...
                <Card className="border-0 shadow-xl bg-white hover:shadow-2xl transition-shadow duration-300">
                  <CardContent className="p-8">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-2">Max Drawdown</p>
                        <p className="text-3xl font-bold text-red-600">
                          {(advancedStats.risk.maxDrawdown * 100).toFixed(1)}%
                        </p>
                        <p className="text-xs text-slate-500 mt-1">Maximum portfolio decline</p>
                      </div>
                ...
                <Card className="border-0 shadow-xl bg-white hover:shadow-2xl transition-shadow duration-300">
                  <CardContent className="p-8">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-2">Annual Return</p>
                        <p className="text-3xl font-bold text-purple-600">
                          {(advancedStats.performance.annualReturn * 100).toFixed(1)}%
                        </p>
                        <p className="text-xs text-slate-500 mt-1">Annualized performance</p>
                      </div>
              ...
              <Card className="border-0 shadow-xl bg-white">
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl font-bold text-slate-800">Cumulative P&L</CardTitle>
                      <CardDescription className="text-slate-600">Your trading performance over time</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
              ...
              <Card className="border-0 shadow-xl bg-white">
                <CardHeader className="pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl font-bold text-slate-800">Trade Distribution</CardTitle>
                      <CardDescription className="text-slate-600">Breakdown of profitable vs losing trades</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-600">Professional trading performance analysis</p>
                </div>
              </div>
                ...
                <Card className="border-0 shadow-xl bg-white">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-xl font-bold text-slate-800">Performance Radar</CardTitle>
                        <CardDescription className="text-slate-600">Multi-dimensional performance analysis</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                ...
                <Card className="border-0 shadow-xl bg-white">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-xl font-bold text-slate-800">Risk Management</CardTitle>
                        <CardDescription className="text-slate-600">Risk control and management assessment</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
              ...
              <Card className="border-0 shadow-xl bg-white">
                <CardHeader>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl font-bold text-slate-800">Performance Timeline ({selectedPeriod})</CardTitle>
                      <CardDescription className="text-slate-600">Trading performance over selected period</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <ResponsiveContainer width="100%" height={450}>
                    <ComposedChart data={timeSeriesData}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                      <XAxis dataKey="period" tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                      <YAxis yAxisId="left" tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                      <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                      <Tooltip 
                        contentStyle={{
                          backgroundColor: 'white',
                          border: '1px solid #e2e8f0',
                      ...
                      />
                      <Legend />
                      <Bar yAxisId="left" dataKey="trades" fill="url(#gradient2)" name="Trades" radius={[4, 4, 0, 0]} />
                      <Line yAxisId="right" type="monotone" dataKey="cumulativePnL" stroke="#f59e0b" strokeWidth={3} name="Cumulative P&L" dot={{ fill: '#f59e0b', strokeWidth: 2, r: 5 }} />
                      <Line yAxisId="right" type="monotone" dataKey="winRate" stroke="#10b981" strokeWidth={3} name="Win Rate %" dot={{ fill: '#10b981', strokeWidth: 2, r: 5 }} />
                      <defs>
            ...
            <Card className="border-0 shadow-xl bg-white">
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-bold text-slate-800">Strategy Performance Analysis</CardTitle>
                    <CardDescription className="text-slate-600">Detailed breakdown of strategy effectiveness</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={chartData.strategyPerformance}>
                    ...
                    <Tooltip 
                      formatter={(value: any, name: any) => [
                        name === 'totalPnL' ? `₹${value.toFixed(2)}` : 
                        name === 'avgPnL' ? `₹${value.toFixed(2)}` : 
                    ...
                    />
                    <Legend />
                    <Bar dataKey="totalPnL" fill="url(#gradient3)" name="totalPnL" radius={[4, 4, 0, 0]} />
                    <Bar dataKey="winRate" fill="url(#gradient4)" name="winRate" radius={[4, 4, 0, 0]} />
                    <defs>
            ...
            <Card className="border-0 shadow-xl bg-white">
              <CardHeader>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl font-bold text-slate-800">Risk-Reward Analysis</CardTitle>
                    <CardDescription className="text-slate-600">Distribution of risk vs reward across trades</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <ResponsiveContainer width="100%" height={400}>
                  <ScatterChart data={chartData.riskDistribution}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                    <XAxis dataKey="risk" name="Risk" tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                    <YAxis dataKey="reward" name="Reward" tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                    <Tooltip 
                      cursor={{ strokeDasharray: '3 3' }}
                      formatter={(value: any, name: any) => [`₹${value.toFixed(2)}`, name === 'risk' ? 'Risk' : 'Reward']}
                      contentStyle={{
                        backgroundColor: 'white',
                        border: '1px solid #e2e8f0',
                        borderRadius: '12px',
                    ...
                    />
                    <Scatter dataKey="reward" fill="#6366f1" stroke="#4f46e5" strokeWidth={2} />
                  </ScatterChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
        ...
        {activeTab === 'patterns' && (
          <div className="space-y-8">
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h2 className="text-3xl font-bold text-slate-800">Trading Patterns</h2>
                <p className="text-slate-600">Discover patterns in your trading behavior</p>
              </div>
            </div>
                  ...
                  <Card className="border-0 shadow-xl bg-gradient-to-br from-green-50 to-emerald-50">
                    <CardContent className="p-8">
                      <div className="text-center">
                        ...
                        <div className="text-lg font-semibold text-slate-700 mb-2">Best Win Streak</div>
                        <div className="text-sm text-slate-500">Consecutive winning trades</div>
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="border-0 shadow-xl bg-gradient-to-br from-red-50 to-pink-50">
                    <CardContent className="p-8">
                      <div className="text-center">
                        ...
                        <div className="text-lg font-semibold text-slate-700 mb-2">Worst Loss Streak</div>
                        <div className="text-sm text-slate-500">Consecutive losing trades</div>
                      </div>
                    </CardContent>
                  </Card>
                </div>
                ...
                <Card className="border-0 shadow-xl bg-white">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-xl font-bold text-slate-800">Day of Week Performance</CardTitle>
                        <CardDescription className="text-slate-600">How you perform on different days</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={400}>
                      <BarChart data={prepareDayOfWeekData()}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" />
                        <XAxis dataKey="day" tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                        <YAxis tick={{ fontSize: 12, fill: '#64748b' }} stroke="#94a3b8" />
                        <Tooltip 
                          formatter={(value: any, name: any) => [
                            name === 'totalPnL' ? `₹${value.toFixed(2)}` : 
                        ...
                        />
                        <Legend />
                        <Bar dataKey="totalPnL" fill="url(#gradient5)" name="totalPnL" radius={[4, 4, 0, 0]} />
                        <Bar dataKey="trades" fill="url(#gradient6)" name="trades" radius={[4, 4, 0, 0]} />
                        <defs>
                ...
                <Card className="border-0 shadow-xl bg-white">
                  <CardHeader>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-xl font-bold text-slate-800">Top Performing Instruments</CardTitle>
                        <CardDescription className="text-slate-600">Your best and worst performing instruments</CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent>
                    <ResponsiveContainer width="100%" height={400}>
                      <BarChart data={prepareInstrumentData()}>
                        ...
                        />
                        <Legend />
                        <Bar dataKey="totalPnL" fill="url(#gradient7)" name="totalPnL" radius={[4, 4, 0, 0]} />
                        <defs>
          ...
          <div className="space-y-8">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-3xl font-bold text-slate-800">AI-Powered Insights</h2>
                  <p className="text-slate-600">Professional trading analysis and recommendations</p>
                </div>
              </div>
              <Button 
                onClick={generateAdvancedAnalytics}
            ...
            {analytics ? (
                  ...
                  <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-50 to-indigo-50">
                    <CardHeader className="pb-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-2xl font-bold text-slate-800">Executive Summary</CardTitle>
                          <CardDescription className="text-slate-600">Comprehensive performance overview</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-6">
                      <div className="p-6 bg-white rounded-xl border border-blue-100 shadow-sm">
                        <h4 className="font-bold text-lg mb-3 text-slate-800">Overall Performance</h4>
                        <p className="text-slate-700 leading-relaxed">{analytics.executive_summary.overall_performance}</p>
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-8">
                        <div className="p-6 bg-white rounded-xl border border-green-100 shadow-sm">
                          ...
                          <ul className="space-y-3">
                            {analytics.executive_summary.key_strengths?.map((strength: string, index: number) => (
                        ...
                        <div className="p-6 bg-white rounded-xl border border-red-100 shadow-sm">
                          ...
                          <ul className="space-y-3">
                            {analytics.executive_summary.critical_weaknesses?.map((weakness: string, index: number) => (
                              <li key={index} className="flex items-start space-x-3">
                      ...
                      {analytics.executive_summary.trader_profile && (
                        <div className="p-6 bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl border border-purple-100">
                          ...
                          <p className="text-slate-700 leading-relaxed">{analytics.executive_summary.trader_profile}</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <Card className="border-0 shadow-xl bg-white">
                    <CardHeader>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-2xl font-bold text-slate-800">Advanced Performance Metrics</CardTitle>
                          <CardDescription className="text-slate-600">Quantitative performance analysis</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {Object.entries(analytics.performance_metrics).map(([key, value]) => (
                          <div key={key} className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100 hover:shadow-lg transition-shadow duration-300">
                            <div className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-2">
                              {key.replace(/_/g, ' ')}
                            </div>
                            <div className="text-2xl font-bold text-blue-700">{String(value)}</div>
                          </div>
                        ))}
                      </div>
                    </CardContent>
                  ...
                  <Card className="border-0 shadow-xl bg-white">
                    <CardHeader>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-2xl font-bold text-slate-800">Risk Analysis</CardTitle>
                          <CardDescription className="text-slate-600">Comprehensive risk assessment</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="grid gap-6">
                        {Object.entries(analytics.risk_analysis).map(([key, value]) => (
                          <div key={key} className="p-6 border-l-4 border-red-400 bg-gradient-to-r from-red-50 to-pink-50 rounded-r-xl">
                  ...
                  <Card className="border-0 shadow-xl bg-white">
                    <CardHeader>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-2xl font-bold text-slate-800">Strategy Analysis</CardTitle>
                          <CardDescription className="text-slate-600">Strategic insights and recommendations</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-6">
                        {Object.entries(analytics.strategy_analysis).map(([key, value]) => (
                          <div key={key} className="p-6 bg-gradient-to-r from-purple-50 to-violet-50 rounded-xl border border-purple-100">
                  ...
                  <Card className="border-0 shadow-xl bg-gradient-to-br from-green-50 to-emerald-50">
                    <CardHeader>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-2xl font-bold text-slate-800">Actionable Recommendations</CardTitle>
                          <CardDescription className="text-slate-600">Strategic action plan for improvement</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-8">
                        {Object.entries(analytics.actionable_recommendations).map(([timeframe, recommendations]) => (
                          <div key={timeframe} className="p-6 bg-white rounded-xl border border-green-100 shadow-sm">
                            <h4 className="font-bold text-xl mb-4 capitalize text-green-700 flex items-center gap-2">
                              <Clock className="w-5 h-5" />
                              {timeframe.replace(/_/g, ' ')}
                            </h4>
                            <ul className="space-y-4">
                              {Array.isArray(recommendations) ? recommendations.map((rec: string, index: number) => (
                                <li key={index} className="flex items-start space-x-4 p-4 bg-green-50 rounded-xl border border-green-100">
                                ...
                                <li className="flex items-start space-x-4 p-4 bg-green-50 rounded-xl border border-green-100">
                                  ...
                                  <span className="text-slate-700 leading-relaxed font-medium">{String(recommendations)}</span>
                                </li>
                              )}
                            </ul>
                          </div>
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-purple-50 to-pink-50">
                <CardContent className="p-16 text-center">
                  ...
                  <h3 className="text-3xl font-bold mb-4 text-slate-800">AI Insights Ready</h3>
                  <p className="text-lg text-slate-600 mb-8 max-w-2xl mx-auto leading-relaxed">
                    Generate comprehensive AI-powered analysis of your trading performance, 
                    risk management, and personalized recommendations tailored to your trading style.
                  </p>
                  <Button 
                    onClick={generateAdvancedAnalytics}
                    disabled={loading || entries.length === 0}
                  ...
                  {entries.length === 0 && (
                    <p className="text-sm text-slate-500 mt-4">Add at least one trade to enable AI analysis</p>
                  )}
                </CardContent>
              </Card>
            )}
          ...
          <div className="space-y-8">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-3xl font-bold text-slate-800">Trade Journal</h2>
                  <p className="text-slate-600">Manage and track your trading positions</p>
                </div>
              </div>
            ...
            {showForm && (
              <Card className="border-0 shadow-2xl bg-white">
                <CardHeader className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-t-xl">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-gradient-to-r from-indigo-500 to-blue-600 rounded-xl flex items-center justify-center">
                      {editingEntry ? <Edit className="w-5 h-5 text-white" /> : <Plus className="w-5 h-5 text-white" />}
                    </div>
                    <div>
                      ...
                      <CardDescription className="text-slate-600">
                        {editingEntry ? 'Update the trade details below' : 'Enter the details of your trade'}
                      </CardDescription>
                    </div>
                  </div>
                </CardHeader>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      <div className="space-y-3">
                        <Label htmlFor="broker" className="text-sm font-semibold text-slate-700">Broker</Label>
                        <Input
                          id="broker"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="market" className="text-sm font-semibold text-slate-700">Market</Label>
                        <Select 
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="instrument" className="text-sm font-semibold text-slate-700">Instrument</Label>
                        <Input
                          id="instrument"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="direction" className="text-sm font-semibold text-slate-700">Direction</Label>
                        <Select 
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="qty" className="text-sm font-semibold text-slate-700">Quantity</Label>
                        <Input
                          id="qty"
                          type="number"
                          value={formData.qty}
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="entry_price" className="text-sm font-semibold text-slate-700">Entry Price</Label>
                        <Input
                          id="entry_price"
                          type="number"
                          step="0.01"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="exit_price" className="text-sm font-semibold text-slate-700">Exit Price (Optional)</Label>
                        <Input
                          id="exit_price"
                          type="number"
                          step="0.01"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="entry_dt" className="text-sm font-semibold text-slate-700">Entry Date & Time</Label>
                        <Input
                          id="entry_dt"
                          type="datetime-local"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="exit_dt" className="text-sm font-semibold text-slate-700">Exit Date & Time (Optional)</Label>
                        <Input
                          id="exit_dt"
                          type="datetime-local"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="stoploss" className="text-sm font-semibold text-slate-700">Stop Loss</Label>
                        <Input
                          id="stoploss"
                          type="number"
                          step="0.01"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="commission" className="text-sm font-semibold text-slate-700">Commission</Label>
                        <Input
                          id="commission"
                          type="number"
                          step="0.01"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="strategy" className="text-sm font-semibold text-slate-700">Strategy (Optional)</Label>
                        <Input
                          id="strategy"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-3">
                        <Label htmlFor="setup" className="text-sm font-semibold text-slate-700">Setup Description</Label>
                        <Textarea
                          id="setup"
                      ...
                      <div className="space-y-3">
                        <Label htmlFor="reason" className="text-sm font-semibold text-slate-700">Reason/Notes</Label>
                        <Textarea
                          id="reason"
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="text-slate-600">Complete record of your trading activity</CardDescription>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="border-b-2 border-slate-200">
                        ...
                        <th className="text-left p-4 font-bold text-slate-700">Instrument</th>
                        <th className="text-left p-4 font-bold text-slate-700">Direction</th>
                        <th className="text-left p-4 font-bold text-slate-700">Qty</th>
                        <th className="text-left p-4 font-bold text-slate-700">Entry</th>
                        <th className="text-left p-4 font-bold text-slate-700">Exit</th>
                        <th className="text-left p-4 font-bold text-slate-700">P&L</th>
                        <th className="text-left p-4 font-bold text-slate-700">Status</th>
                        <th className="text-left p-4 font-bold text-slate-700">Strategy</th>
                        <th className="text-left p-4 font-bold text-slate-700">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filteredEntries.map((entry) => (
                        <tr key={entry.s_no} className="border-b border-slate-100 hover:bg-slate-50 transition-colors duration-200">
                          <td className="p-4 text-sm font-medium text-slate-700">
                            {new Date(entry.entry_dt).toLocaleDateString()}
                          </td>
                          <td className="p-4 font-semibold text-slate-800">{entry.instrument}</td>
                          <td className="p-4">
                          ...
                          <td className="p-4 font-medium text-slate-700">{entry.qty}</td>
                          <td className="p-4 font-mono font-medium text-slate-700">₹{entry.entry_price.toFixed(2)}</td>
                          ...
                          <td className="p-4">
                            {entry.p_and_l !== undefined ? (
                              <span className={`font-bold font-mono ${
                                entry.p_and_l >= 0 ? 'text-green-600' : 'text-red-600'
                              }`}>
                          ...
                          <td className="p-4 text-sm font-medium text-slate-600">{entry.strategy}</td>
                          <td className="p-4">
