...
function TradingJournalApp() {
  ...
  // API helper function - UPDATED with proper authentication
  ...
  const fetchEntries = async () => {
    if (!authState.session?.access_token) {
      console.log('No access token available for fetching entries');
      return;
    }
    
    try {
      const response = await apiCall<{ trades: JournalEntry[]; count: number }>(
        `/trades?limit=1000`
      );
      ...
      if (response.trades) {
        const formattedEntries = response.trades.map((entry: JournalEntry) => ({
          ...entry,
          entry_dt: entry.entry_dt ? new Date(entry.entry_dt).toISOString().slice(0, 16) : '',
      ...
      // Don't show error to user on initial load failure
      setEntries([]);
    }
  };
  ...
  const fetchAdvancedStats = async () => {
    ...
    try {
      ...
      if (response.stats) {
        ...
        console.log('✅ Fetched advanced stats');
      }
    } catch (error) {
  ...
  const fetchTimeSeriesData = async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        console.log(`✅ Fetched time series data for ${selectedPeriod}`);
      }
    } catch (error) {
  ...
  // Initialize trading data after authentication
  const initializeTradingData = async () => {
    if (!authState.session?.access_token) {
      console.log('No access token available for initializing data');
      return;
    }
    ...
    try {
      ...
      if (connected) {
        // Fetch all data in sequence to avoid overwhelming the server
        await fetchEntries();
        await fetchStatistics();
        await fetchAdvancedStats();
        await fetchTimeSeriesData();
        await fetchPatternData();
        console.log('✅ All trading data initialized successfully');
      } else {
        console.warn("Unable to connect to the backend API.");
        setError("Unable to connect to the trading journal server. Please try refreshing the page.");
      }
    } catch (error) {
      console.error('Failed to initialize trading data:', error);
      setError("Failed to load your trading data. Please try refreshing the page.");
    } finally {
      setLoading(false);
    }
  };
      ...
      try {
        ...
        if (isMounted) {
          if (session?.user) {
            ...
            // Initialize trading data after setting auth state
  ...
  useEffect(() => {
    if (authState.user && authState.session && !loading) {
      initializeTradingData();
    }
  }, [authState.user, authState.session]); // Run when user or session changes
  ...
  useEffect(() => {
    if (apiConnected && authState.session?.access_token) {
      fetchTimeSeriesData();
    }
  }, [selectedPeriod, apiConnected, authState.session?.access_token]);

  // Save entry to API
  const saveEntry = async (entry: Omit<JournalEntry, "s_no">) => {
    try {
      const response = await apiCall<{ trade: JournalEntry; message: string }>("/trades", {
        method: "POST",
      ...
      if (response.trade) {
        const formattedEntry = {
          ...response.trade,
          entry_dt: response.trade.entry_dt
            ? new Date(response.trade.entry_dt).toISOString().slice(0, 16)
            : '',
  ...
  const updateEntry = async (entry: JournalEntry) => {
    try {
      const response = await apiCall<{ trade: JournalEntry; message: string }>(
        `/trades/${entry.s_no}`,
        {
          method: "PUT",
          body: JSON.stringify(entry),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!apiConnected || !authState.session?.access_token) {
      setError("API connection not available or user not authenticated. Please refresh the page or sign in again.");
      return;
    }
  ...
  // Handle delete with data refresh
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                {!apiConnected && (
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 border border-red-200">
                ...
                {apiConnected && (
                  <div className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold bg-green-100 text-green-800 border border-green-200">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {loading && (
          <div className="text-center py-12">
            ...
            <p className="text-slate-600 font-medium">Loading your trading data...</p>
          </div>
        )}
        ...
        {activeTab === "dashboard" && (
          <div className="space-y-8">
            ...
            {entries.length === 0 && !loading && (
              <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-50 to-indigo-50">
                <CardContent className="p-12 text-center">
                  <div className="w-20 h-20 bg-gradient-to-r from-indigo-600 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl">
                    <TrendingUp className="w-10 h-10 text-white" />
                  </div>
                  ...
                  <p className="text-lg text-slate-600 mb-8 max-w-2xl mx-auto leading-relaxed">
                    Your professional trading journal is ready. Start by adding your first trade to unlock powerful analytics, AI insights, and performance tracking features.
                  </p>
                  <Button
            ...
            {/* Premium Performance Charts - Only show if we have data */}
            {entries.length > 0 && (
        ...
        {activeTab === "analytics" && (
          <div className="space-y-8">
              ...
              <Card className="border-0 shadow-xl bg-gradient-to-br from-blue-50 to-indigo-50">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-lg text-slate-600 mb-8 max-w-xl mx-auto leading-relaxed">
                    Add some trades to your journal to unlock powerful analytics and insights about your trading performance.
                  </p>
                  <Button
            ...
            {advancedStats && entries.length > 0 && (
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <Card className="border-0 shadow-xl bg-white">
                  <CardHeader>
                    <div className="flex items-center gap-3">
          ...
          <div className="space-y-8">
            ...
            {entries.length === 0 && !loading && (
              <Card className="border-0 shadow-xl bg-gradient-to-br from-purple-50 to-violet-50">
                <CardContent className="p-12 text-center">
                  <div className="w-16 h-16 bg-gradient-to-r from-purple-500 to-violet-600 rounded-2xl flex items-center justify-center mx-auto mb-6 shadow-xl">
                  ...
                  <p className="text-lg text-slate-600 mb-8 max-w-xl mx-auto leading-relaxed">
                    Add some trades to discover interesting patterns in your trading behavior and performance.
                  </p>
                  <Button
                    onClick={() => setActiveTab("trades")}
                    className="bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700 text-white px-8 py-4 text-lg font-semibold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300"
                  >
                    <Plus className="w-6 h-6 mr-3" />
                    Go to Trades
                  </Button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <Input
                          ...
                          placeholder="e.g.,EURUSD, RELIANCE, BTC"
                          className="h-12 border-2 border-slate-200 rounded-xl focus:border-indigo-500"
                          required
                        />
                      </div>
