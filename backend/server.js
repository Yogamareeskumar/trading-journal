...
const supabaseServiceKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZzaWNhdWNlb3NtZHJoeG12cmV1Iiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1ODEwMjk5OSwiZXhwIjoyMDczNjc4OTk5fQ.bqzxqGvx_l8-PQ4Ms5fgorweqQCn8fWaBF1O8fs8lX0'; // Replace with your actual key
const supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZzaWNhdWNlb3NtZHJoeG12cmV1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTgxMDI5OTksImV4cCI6MjA3MzY3ODk5OX0.J_Dx9SLkzffTFcDhxMix56cmtpM4710nqafnyP5BLhk'; // Replace with your actual key
...
app.put('/api/auth/profile', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    const result = await pool.query(updateQuery, [
      userId, phone, tradingExperience, preferredMarket, riskTolerance
    ]);
...
app.post('/api/trades', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    const {
      status, broker, market, instrument, direction, qty,
    ...
    const values = [
      userId, status, broker, market, instrument, direction, qty,
...
app.put('/api/trades/:id', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    const values = [
      tradeId, status, broker, market, instrument, direction, qty,
      entry_price, exit_price || null, entry_dt, exit_dt || null,
      stoploss, commission, p_and_l || null, strategy, setup || null, reason || null, userId
    ];
...
app.delete('/api/trades/:id', authenticateSupabaseUser, async (req, res) => {
  try {
    ...
    const query = 'DELETE FROM trading_journal WHERE s_no = $1 AND user_id = $2 RETURNING s_no';
